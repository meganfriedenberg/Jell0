...
struct spring // included kD and kHook since they're different for collision springs
{
    struct point p1; // first point the spring connects aka A
    struct point p2; // second point the spring connects aka B
    double restingLength; // resting length of the spring
    double kHook; // kHooke coefficient for the spring
    double kD; // damping coefficient
};

struct structuralSprings
{
    std::vector<struct spring>* springs; // not sure how many there are
};

extern bool initializeSprings;

extern struct structuralSprings structSpringList;
...
extern struct bendSprings bendSpringList;
...
extern struct shearSprings shearSpringList;
...
// added dot product
// struct point, struct point, dest
#define pDOT(src1,src2,dest)\
\
  (dest) = (src1).x * (src2).x;\
  (dest) = (dest) + (src1).y * (src2).y;\
  (dest) = (dest) + (src1).z * (src2).z;
